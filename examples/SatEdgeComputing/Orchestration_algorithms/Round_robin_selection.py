...
class RoundRobinSelection(Selection):
    def __init__(self, isl_manager):
        super(RoundRobinSelection, self).__init__()
    ...
    def get_path(self, sim, app_name, message, topology_src, alloc_DES, alloc_module, traffic, from_des):
        ...
        # Seleção com base em menor carga
        load_by_node = {node: self.counter[node] for node in candidate_nodes}
        min_load = min(load_by_node.values())
        min_nodes = [node for node, load in load_by_node.items() if load == min_load]
