...
class TradeOffSelection(Selection):
    def __init__(self, isl_manager):
        super(TradeOffSelection, self).__init__()
    ...
    def get_path(self, sim, app_name, message, topology_src, alloc_DES, alloc_module, traffic, from_des):
        ...
        for node in candidate_nodes:
            # 1. Distância (usando a função já existente)
            ...
            # 3. Número de tarefas (carga atual)
            load = self.counter[node]

            # 4. Poder computacional (IPT)
            ipt = G.nodes[node].get("IPT", 1)
            ipt_delay = message.inst / float(ipt)


            # Avaliação Trade-Off
            score = distance_delay * energy * load * ipt_delay
            trade_off_scores[node] = score

        chosen = min(trade_off_scores, key=trade_off_scores.get)
