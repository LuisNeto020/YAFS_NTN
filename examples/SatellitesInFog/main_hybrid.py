...
import math
from placements.placement_for_hybrid import CloudCentricPlacement
from populations.population_for_hybrid import EvolPop
...
def run_simulation(folder_results):
    ...
    _ = sat.initial_sat_info()
    s.deploy_satellite_mobility(sat)
    
    """
    APPLICATION or SERVICES
    """
    data_app = json.load(open('data/appDefinition.json'))
    ...
    selector_path = WeightGreedySelectionSat()
    
    for _, a_name in enumerate(apps.keys()):
        s.deploy_app2(apps[a_name], placement, population, selector_path)
