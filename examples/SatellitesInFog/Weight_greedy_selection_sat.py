...
class WeightGreedySelectionSat(Selection):
    def __init__(self, weights=(0.5, 0.3, 0.2)):
        super(WeightGreedySelectionSat, self).__init__()
    ...
    def get_path(self, sim, app_name, message, topology_src, alloc_DES, alloc_module, traffic, from_des):
        ...
        # Se nó atual for SATELLITE, deve enviar para CLOUD
        if msg_type == "raw_data_to_cloud" and src_type == "MOBILE":
            neighbors = list(G.neighbors(src))
            if G.nodes[neighbors[0]]["type"] == "STATIC":                                                                       
                candidate_fog_nodes = [n for n in G.nodes if G.nodes[n].get("type") == "CLOUD" or G.nodes[n].get("type") == "STATIC"]
            else:
                candidate_fog_nodes = [n for n in G.nodes if G.nodes[n].get("type") == "SATELLITE" or G.nodes[n].get("type") == "CLOUD"]
            print(f"Candidate fog nodes for {src} to {dst_service}: {candidate_fog_nodes}")
            stats = []
            valid_candidates = []
            ...
            for node in candidate_fog_nodes:
                try:
                    ...
                    ipt = G.nodes[node]["IPT"]  # IPT do nó candidato
            ...
            if not stats:
                print(f"[ERROR] No valid candidates found for {src} to {dst_service}")
                return [], None
